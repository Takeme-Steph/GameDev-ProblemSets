...
function AlienLaunchMarker:init(world)
    ...
    -- keep track of if player is still in the air for this round (has not hit anything)
    self.Phit = false
end
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            table.insert(self.alien,Alien(self.world, 'round', self.shiftedX, self.shiftedY, 'Player'))  
    ...
    -- split into 3 players when launched and not yet hit anything
    elseif love.keyboard.wasPressed('space') and self.launched and not self.Phit then
        -- get position and velocity of current player
        local pY, pX = self.alien[1].body:getY(), self.alien[1].body:getX()
        local xVel, yVel = self.alien[1].body:getLinearVelocity()
        
        -- create an extra player above
        local a = Alien(self.world, 'round', pX, pY - 100, 'Player')
        a.body:setLinearVelocity(xVel, yVel)
        a.fixture:setRestitution(0.4)
        a.body:setAngularDamping(1)

        -- create an extra player below
        local b = Alien(self.world, 'round', pX, pY + 100, 'Player')
        b.body:setLinearVelocity(xVel, yVel)
        b.fixture:setRestitution(0.4)
        b.body:setAngularDamping(1)

        table.insert(self.alien, a)
        table.insert(self.alien, b)

        -- prevent spawning another pair
        self.Phit = true
    end
end
