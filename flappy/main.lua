...
--store game pause status
gPAUSED = false
--used to determine if game was just unpaused
gPauseSwitch = false
...
function love.keypressed(key)
    ...
    --Press P to update game pause status
    if key == 'p' then
        if gPAUSED == false then
            gPAUSED = true
            love.audio.pause(sounds['music'])
            love.audio.play(sounds['pause'])
        elseif gPAUSED == true then
            gPAUSED = false
            gPauseSwitch = true
        end
    end
...
function love.update(dt)
    ...
    --stop scrolling if game is paused
    if not gPauseSwitch and not gPAUSED then
